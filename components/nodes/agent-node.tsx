...
function AgentNode({ data, selected, id }: NodeProps<NodeData>) {
  ...
  const nodeWidth = useFixedNodeSize(data.label, {
    extraWidthForMetadata: data.agentType ? 40 : 0,
  ...
  return (
    <div className="relative" onDoubleClick={handleDoubleClick} style={{ width: `${nodeWidth}px`, height: "110px" }}>
      ...
      <div className="pt-4 pb-6">
        <Shape type="hexagon" width={nodeWidth} height={100} fill="#fef9c3" stroke="#eab308" strokeWidth={2} />
      </div>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        {isEditing ? (
          <input
            ...
            className="w-4/5 bg-transparent text-sm font-medium outline-none border-b border-yellow-500 text-center text-gray-600"
          />
        ) : (
          <div className="px-12 text-sm font-medium text-center max-w-full flex flex-col justify-center h-full">
            ...
            {data.agentType && (
              <div className="mt-1 text-xs text-gray-500">
                Type: <span className="font-medium capitalize">{data.agentType}</span>
              </div>
            )}
          </div>
        )}
