...
function DomainPropertyNode({ data, selected, id }: NodeProps<NodeData>) {
  ...
  const handleBlur = () => {
    ...
    if (label.trim() !== data.label) {
      data.label = label.trim() || "Domain Property"
    }
  }
  ...
  return (
    <div className="relative" onDoubleClick={handleDoubleClick} style={{ width: `${nodeWidth}px`, height: "80px" }}>
      ...
      {selected && (
        <div className="absolute -inset-2 rounded-md border-2 border-blue-700 border-dashed pointer-events-none" />
      )}

      {/* Add margin for connection edges */}
      <div className="pt-4 pb-6">
        <Shape type="trapezoid" width={nodeWidth} height={60} fill="#dbeafe" stroke="#1d4ed8" strokeWidth={2} />
      </div>
      ...
      <Handle
        ...
        className="!bg-blue-700 react-flow__handle-top"
        style={{ top: -4 }} // Move handle up by 4px
      />
      <Handle
        type="source"
        position={Position.Bottom}
        className="!bg-blue-700 react-flow__handle-bottom"
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        {isEditing ? (
          <input
            ...
            className="w-4/5 bg-transparent text-sm font-medium outline-none border-b border-blue-700 text-center text-gray-600"
          />
        ) : (
...
export default memo(DomainPropertyNode)
