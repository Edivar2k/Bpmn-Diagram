...
function ObstacleNode({ data, selected, id }: NodeProps<NodeData>) {
  ...
  const nodeWidth = useFixedNodeSize(data.label, {
    extraWidthForMetadata: data.severity ? 40 : 0,
  ...
  const handleBlur = () => {
    ...
    if (label.trim() !== data.label) {
      data.label = label.trim() || "Obstacle"
    }
  }
  ...
  return (
    <div className="relative" onDoubleClick={handleDoubleClick} style={{ width: `${nodeWidth}px`, height: "80px" }}>
      ...
      {selected && (
        <div className="absolute -inset-2 rounded-md border-2 border-orange-500 border-dashed pointer-events-none" />
      )}
      ...
      <Handle
        ...
        className="!bg-orange-500 react-flow__handle-top"
        style={{ top: -4 }} // Move handle up by 4px
      />
      <Handle
        type="source"
        position={Position.Bottom}
        className="!bg-orange-500 react-flow__handle-bottom"
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        {isEditing ? (
          <input
            ...
            className="w-4/5 bg-white/80 text-sm font-medium outline-none border-b border-orange-500 text-center text-gray-600 rounded px-1"
          />
        ) : (
...
export default memo(ObstacleNode)
